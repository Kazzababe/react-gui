...
public class MaskedMenuPartition extends MenuPartition<MaskedMenuPartition> {
    ...
    private @Nullable MenuPartitionItem getItemAtSlot(final int slot) {
        for (final MenuPartitionItem partitionItem : this.items) {
            if (partitionItem.slot() == slot) {
    ...
    private int getNextSlot() {
        ...
        for (int i = 0, slot = 0; i < this.mask.length(); i++, slot++) {
            ...
            final MenuPartitionItem partitionItem = this.getItemAtSlot(itemSlot);
